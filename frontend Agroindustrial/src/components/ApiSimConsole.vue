<script setup>
import { ref } from 'vue';
import { useApiSimLog } from '../composables/useApiSimLog';
...
<template>
  <button
    @click="log.toggle()"
    class="fixed bottom-5 right-5 z-[9998] flex items-center gap-2 rounded-full bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-900/40 px-4 py-3 transition-all cursor-pointer"
  ...
  >
    ...
    <span v-if="log.events.value.length > 0" class="flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full bg-white text-violet-700 text-xs font-bold">
      {{ log.events.value.length }}
    </span>
  </button>

  <div v-if="log.isOpen.value" class="fixed inset-0 z-[9999] flex justify-end">
    ...
    <div class="relative w-full max-w-md h-full bg-slate-950 text-slate-100 border-l border-white/10 flex flex-col shadow-2xl">
      ...
      <div class="flex-1 overflow-y-auto divide-y divide-white/5">
        <p v-if="log.events.value.length === 0" class="text-xs text-slate-500 p-4">
          Aún no hay actividad. Interactúa con la app (crea una maquinaria, cierra una orden de trabajo...) para ver aquí las peticiones simuladas.
        </p>
        <div v-for="event in log.events.value" :key="event.id" class="px-4 py-3">
