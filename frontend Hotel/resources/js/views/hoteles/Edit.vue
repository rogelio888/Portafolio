<!-- resources/js/views/hoteles/Edit.vue -->
...
<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <h1 class="text-3xl font-bold text-gray-800">Editar Hotel</h1>
      <p class="text-gray-600 mt-1">Actualiza la información del hotel</p>
    </div>
...
const fetchHotel = async () => {
  cargando.value = true;
  try {
    const hotel = await hotelesStore.fetchHotel(route.params.id);
    form.value = {
      nombre: hotel.nombre,
    ...
    error.value = 'Error al cargar hotel';
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    await hotelesStore.updateHotel(route.params.id, form.value);
    router.push('/hoteles');
  } catch (err) {
    error.value = err.response?.data?.message || 'Error al actualizar hotel';
  } finally {
    loading.value = false;
  }
};
