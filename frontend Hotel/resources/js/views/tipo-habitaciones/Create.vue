<template>
  <div class="p-4 sm:p-6">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Crear Tipo de Habitación</h1>
      <router-link
    ...
    <form @submit.prevent="guardar" class="space-y-4 max-w-lg">
      <div>
...
const resetForm = () => {
  form.value = { nombre: '', descripcion: '', capacidad: 1, precio_base: 0, estado: 'ACTIVO' };
};

const guardar = async () => {
  loading.value = true;
  try {
    await axios.post('/tipo-habitaciones', form.value);
