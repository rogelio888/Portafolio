<!-- resources/js/views/empleados/Edit.vue -->
...
<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <p class="text-gray-600 mt-1">Modifica los datos del empleado</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 sm:p-6 max-w-3xl">
      ...
      <form v-else @submit.prevent="handleSubmit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Contraseña <span class="text-gray-400 text-xs">(Dejar en blanco para no cambiar)</span>
            </label>
            <input
...
const fetchEmpleado = async () => {
  ...
  try {
    const response = await axios.get(`/empleados/${route.params.id}`);
    if (response.data.success) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.put(`/empleados/${route.params.id}`, dataToSend);
    if (response.data.success) {
      router.push('/empleados');
    }
  } catch (err) {
    error.value = err.response?.data?.message || err.response?.data?.errors?.usuario?.[0] || 'Error al actualizar empleado';
  } finally {
    loading.value = false;
  }
};
